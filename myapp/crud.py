...
from schema import CreateAndUpdateCar
...
def get_car_info_by_id(session: Session, id: int) -> CarInfo:
    car_info = session.query(CarInfo).get(id)
...
def delete_car_info(session: Session, _id: int):
    ...
    return f'Car with id {_id} has been deleted'
